...
```{r}
# 1. Get the list of child IDs
child_ids <- participant_centroids %>% 
  filter(age_group == "child") %>% 
  pull(subject_id)

# 2. Run the LOO loop using your "get_mean_group_distance" function
loo_results <- map_df(child_ids, function(exclude_id) {
  
  # Filter out one child
  temp_df <- participant_centroids %>% 
    filter(age_group == "child", subject_id != exclude_id)
  
  # Calculate mean distance for the remaining 17
  dist_val <- get_mean_group_distance(temp_df) 
  
  tibble(excluded_subject = exclude_id, remaining_mean_dist = dist_val)
})

# 3. Check the stability
summary(loo_results$remaining_mean_dist)
```
